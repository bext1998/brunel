...
- v1.2 AC-7 的 stale-read 防護（#5）核心邏輯與單元測試已合併（PR #26），並經 #4（PR #33）接上實際 `workspace.Workspace` 與 8 工具呼叫路徑、Windows AC-6 閉環測試通過，`brunel --taylor-tool` 子行程進入點亦已合併（PR #37）；但真實 `pi --mode rpc` → `--taylor-tool` 這條完整路徑的 E2E／Integration 驗證待 #9（AC-7／AC-6 正式判定需待該閉環）。
