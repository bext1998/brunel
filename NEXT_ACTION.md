...
ADR-002 的架構轉向已拆解完成（#29／#30 拆出、#8／#9 改標題與範圍），`docs/spec.md` 修訂至 v1.3.1（INV-6 best-effort、OQ-10）。#5（F-4，`internal/filetools`）、#7（F-6，`internal/safety`）與 #8（F-7，`internal/pirpc`＋`internal/redact`）的核心實作已分別經 PR #26／PR #27／PR #28 合併至 `main`。#4 的 `internal/tools` 核心已完成於本機 `agent/tools-issue-4` 分支，待 review 與使用者授權的 Git 操作；下一步處理 #30。

## 優先行動

1. Review #4（F-3）工作樹：`internal/tools` 已提供 8 工具 registry、schema snapshot、`safety.Gate` 前置裁決、filetools／exec wiring、`list_files`／`search_text`／Git-only `workspace_diff` 與 Windows AC-6 閉環。經使用者授權後再 commit、push、建立 PR；PR body 必須使用 `Related to #4`，不可 `Closes #4`。完成後解除 #29、#14、部分 #2。
